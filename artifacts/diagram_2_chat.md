# Диаграмма 2: Sequence Diagram - Аутентификация OAuth

## Промпт:
Представь, что ты системный архитектор. Напиши мне код для Mermaid диаграммы типа Sequence, которая описывает процесс аутентификации через OAuth между Клиентом, Приложением и OAuth сервером. Показать запрос кода, получение токена, проверку токена и возврат данных пользователя. Обязательно добавь участника или комментарий с подписью 'Author: GeoExe'.
